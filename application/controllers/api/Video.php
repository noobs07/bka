...
class Video extends REST_Controller {
	...
	function index_get() {
		...
		$columns = array('id_video','judul','cover','is_shown','link');
		$this->db->select('id_video,judul,concat("'.base_url('assets/uploads/video/').'",cover) as cover,is_shown,link');
		if(isset($keyword) && $keyword['value'] != '') {
			$this->db->or_like('judul', $keyword['value']);
			$this->db->or_like('cover', $keyword['value']);
			$this->db->or_like('is_shown', $keyword['value']);
	...
	function save_post(){
		...
		$this->upload_file_config('./assets/uploads/video/', $new_name);
		...
		if ($this->upload->do_upload('file')) {
			...
			$old = $this->get_one($id);
			if ($old) {
				unlink('./assets/uploads/video/'.$old->foto);
			}
		}
